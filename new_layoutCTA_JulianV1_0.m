    ...
    function Layoutcomponents
        ...
        % add contents
            ...
            % in tabs
                ...
                hs.overlay=uicontrol('Style', 'checkbox', 'Parent',hs.DetectTabBox,'String','Overlay images', 'Callback', @Checkboxchange,'FontSize',15);
                hs.overlay=uicontrol('Parent',hs.DetectTabBox,'String','Register images', 'Callback', @RegisterImage,'FontSize',15);
...
%% load, save, refresh and data handling
    ...
    function errorloading=chngDir 
        ...
        if ~isempty(p.l) %found files
            for h=1:size(p.l,1)
            keep(h)=(p.l(h).name(1)~='.'); %removes all directories and parents (files which start with '.')
            end
            p.l=p.l(keep);
        end
        ...
        if ~isempty(dir([p.dir, '/', '*','all','*'])); %found a file countaing "all"
            ...
            hs.UserMess.String='found previous analysis, loaded it into Matlab';drawnow
        elseif exist ([p.dir '/sidesave.mat'], 'file')&& exist ([p.dir '/stoped_at.mat'], 'file') %check for an older saved analysis
            ...
            p.iold=p.oldiload.i;
            set(hs.UserMess, 'String', 'found previous analysis, loaded it into Matlab');
        ...
        p.i=p.iold; p.iOverlay=p.iOverlayold; 
        p.iold=0; %this ensures refresh is going to print the image.
        p.dirOverlay='';
    ...
    function refresh(z)
        ...
        if p.overlayIMGstatus==1
            hold on;
            p.imOverlay=imshow(p.rgbOverlay,'InitialMagnification', 40);
        ...
        p.iold=p.i; p.iOverlayold=p.iOverlay;
...
%% navigate and view images
    ...
    function RegisterImage(~,~)
        %this functions tries to align similar images
        
        %% work with smaller images
        Original=imresize(p.rgb, 0.2); Original=Original(:,:,p.color);
        Overlay=imresize(p.rgbOverlay, 0.2); Overlay=Overlay(:,:,p.color);
        
        %% define an optimizer
        [optimizer, metric]  = imregconfig('monomodal');
        
        %% get the transform
        tform=imregtform(Overlay,Original, 'rigid',optimizer, metric);
        
        %% apply the transform to the full image
        % imwarp...
        p.rgbOverlay=imwarp(p.rgbOverlay,tform, 'OutputView',imref2d(size(p.rgb)));
    end
...
%% graphical interface functions
    ...
    function ScrollWheelFcn(~,eventdata)
        speedlvl=2; % this is to choose how fast it zooms in/out. Behavior is weird if spdde it not 2
...
%% auto detect colonies
    ...
    function FindColoniesAll_Callback(~,~) % --- Executes on button press in AnalyseAllImages.
        ...
        while p.i<length(p.l)+1
            ...
            text(0.25, 0.5, txtMsg,'Fontsize', 14);
        end
...
%% image analysis funtions
    function output=Sauvola(image, varargin)
        %SAUVOLA local thresholding. Done on the Green channel. Good for
        %   staph colonies on 
    ...
    function output=midgrey(image, varargin)
